...
def generate_map(coordinates):
    ...
    print(route1['features'][0]["properties"]["summary"])
...
#from_to_file = open("from_to.json")
#from_to = json.load(from_to_file)
#fromm = from_to["from"]
#to = from_to["to"]
...
command = ("select lat from gradovi where ime like \""+fromm+"\";")
startlat = conn.execute(command).fetchall()[0][0]
command = ("select lon from gradovi where ime like \""+fromm+"\";")
startlon = conn.execute(command).fetchall()[0][0]
command = ("select lat from gradovi where ime like \""+to+"\";")
destlat = conn.execute(command).fetchall()[0][0]
command = ("select lon from gradovi where ime like \""+to+"\";")
